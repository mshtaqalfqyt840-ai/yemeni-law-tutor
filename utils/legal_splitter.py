...
def split_law_by_article(full_text: str, source_name: str) -> list[Document]:
    ...
    1. يستخرج النص التمهيدي والمبادئ العامة في بداية الملف (parts[0]) كمستند مستقل.
    2. يقسّم المواد القانونية بدقة مع دعم التطويل/الكشيدة (المــادة/المادة/مادة رقم).
    3. يحفظ رقم المادة والباب/الفصل الحالي كبيانات وصفية (metadata).
    """
    documents = []
    current_book = "المبادئ العامة والقواعد الكلية"
    current_chapter = ""

    header_regex = re.compile(r'^\s*((?:الكتاب|الباب|القسم|الفصل)\s+[^\n]+)', re.MULTILINE)
    # دعم التطويل والكشيدة (\u0640*) والصيغ المختلفة (مادة / المادة / المــادة / مادة رقم)
    article_pattern = re.compile(r'(?:الم[\u0640]*ادة|م[\u0640]*ادة)\s*(?:رقم\s*)?\(?\s*([\d\u0660-\u0669]+)\s*\)?\s*:?', re.UNICODE)
    ...
    # 2. تقشير واستخراج كل مادة مع الحفاظ على الهيكل التنظيمي كاملًا
    for i in range(1, len(parts), 2):
        ...
        # تحديث الباب/الفصل بناءً على العناوين السابقة للمادة
        ...
        section_label = f"{current_book}" + (f" - {current_chapter}" if current_chapter else "")

        documents.append(Document(
            page_content=f"مادة ({raw_article_number}): {article_text}",
            metadata={
